...
export default function TareaForm({ proyectoId, tareaEditar, onGuardar, onCancelar }: TareaFormProps) {
  ...
  const [tareaPadreId, setTareaPadreId] = useState<number | ''>('');
  ...
  useEffect(() => {
    if (tareaEditar) {
      ...
      setTareaPadreId(tareaEditar.tareaPadreId ?? '');
    }
  }, [tareaEditar]);
  ...
  const handleSubmit = async () => {
    ...
    const datos: TareaRequest = {
      ...
      evidencias: evidencias
        ...
        : [],
      tareaPadreId: tareaPadreId ? Number(tareaPadreId) : null,
    };
    ...
    <div className="modal-overlay" onClick={onCancelar}>
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '480px', width: '90%' }}>
        ...
        <div style={{ maxHeight: '70vh', overflowY: 'auto', paddingRight: '4px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {/* RF-15: Selección de Tarea Padre */}
          <div className="ws-field" style={{ margin: 0 }}>
            <label className="form-label">Tarea Padre (Si es una Subtarea)</label>
            <select
              className="form-input"
              value={tareaPadreId}
              onChange={e => setTareaPadreId(e.target.value ? Number(e.target.value) : '')}
            >
              <option value="">Ninguna (Es una tarea principal)</option>
              {tareasProyecto
                .filter(t => !tareaEditar || t.id !== tareaEditar.id)
                .filter(t => !t.tareaPadreId) // Solo permitir que tareas principales sean padres, evitando jerarquías infinitas
                .map(t => (
        ...
        <div className="modal-botones" style={{ marginTop: '20px', paddingTop: '12px', borderTop: '1px solid var(--border)' }}>
          ...
          <button className="btn-primario" onClick={handleSubmit} disabled={guardando}>
